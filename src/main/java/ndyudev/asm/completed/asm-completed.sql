...
CREATE TABLE Employees (
    Id          NVARCHAR(50) NOT NULL PRIMARY KEY, -- Khóa chính (String -> NVARCHAR)
    Fullname    NVARCHAR(100) NOT NULL,            -- Họ tên (String -> NVARCHAR)
    Gender      BIT NOT NULL,                      -- Giới tính (Boolean -> BIT: 1=Nam, 0=Nữ)
    Salary      FLOAT NOT NULL                     -- Lương (Double -> FLOAT)
);

-- Thêm một vài dữ liệu mẫu để test
INSERT INTO Employees (Id, Fullname, Gender, Salary) VALUES
(N'NV01', N'Nguyễn Văn Tèo', 1, 500.0),
(N'NV02', N'Trần Thị Nở', 0, 1500.0),
(N'NV03', N'Lê Văn Luyện', 1, 5000.0),
(N'NV04', N'Thị Mầu', 0, 2500.0),
(N'NV05', N'Chí Phèo', 1, 3500.0);
...
CREATE TABLE employeess (
    id INT NOT NULL,
    name NVARCHAR(50) NOT NULL, -- Dùng NVARCHAR để lưu tiếng Việt
    salary DECIMAL(10, 2),      -- Dùng DECIMAL cho tiền lương
    dp_id VARCHAR(10),
    PRIMARY KEY (id),
    -- Tạo khóa ngoại liên kết với bảng departments
    FOREIGN KEY (dp_id) REFERENCES departments(id)
);
GO